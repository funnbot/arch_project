...
Here’s an example of the use of [`ask`](dictionary.html#ask) in a NetLogo procedure:
...
-   Check whether two agentsets are equal using [`=`](dictionary.html#Symbols) or [`!=`](dictionary.html#Symbols).
...
The [`replace-item`](dictionary.html#replace-item) reporter takes three inputs. The first input specifies which item in the list is to be changed. 0 means the first item, 1 means the second item, and so forth.
...
Note that each of these special primitives has a default number of inputs for which no parentheses are required. The primitives which have this capability are [`list`](dictionary.html#list), [`word`](dictionary.html#word), [`sentence`](dictionary.html#sentence), [`map`](dictionary.html#map), [`foreach`](dictionary.html#foreach), [`run`](dictionary.html#run), and [`runresult`](dictionary.html#run).
...
The thickness of the lines in the link shape is controlled by the [`thickness`](dictionary.html#thickness) link variable.
...
You don’t have to use the tick counter to use tick-based updates. If the tick counter never advances, the view will update only when you use the [`display`](dictionary.html#display) command.
...
#### [Temporary plot pens](#temporary-plot-pens)
...
Or, if you know you just want to close every file, you can use [`file-close-all`](dictionary.html#file-close-all).
...
Lines drawn by turtles are normally one pixel thick. If you want a different thickness, set the [`pen-size`](dictionary.html#pen-size) turtle variable to a different number before drawing (or erasing). In new turtles, the variable is set to 1.
...
To understand the difference between [`ask`](dictionary.html#ask) and [`ask-concurrent`](dictionary.html#ask-concurrent), consider the following two commands:
...
With [`ask`](dictionary.html#ask), the first turtle takes five steps forward, then the second turtle takes five steps forward, and so on.

With [`ask-concurrent`](dictionary.html#ask-concurrent), all of the turtles take one step forward. Then they all take a second step, and so on. Thus, the latter command is equivalent to:

``` netlogo
repeat 5 [ ask turtles [ fd 1 ] ]
```

> **Code Example:** Ask-Concurrent Example shows the difference between [`ask`](dictionary.html#ask) and [`ask-concurrent`](dictionary.html#ask-concurrent).

The behavior of [`ask-concurrent`](dictionary.html#ask-concurrent) cannot always be so simply reproduced using [`ask`](dictionary.html#ask), as in this example. Consider this command:

``` netlogo
ask-concurrent turtles [ fd random 10 ]
```

In order to get the same behavior using [`ask`](dictionary.html#ask), we would have to write:
...
To prolong an agent’s “turn”, use the [`without-interruption`](dictionary.html#without-interruption) command. (The command blocks inside some commands, such as [`create-turtles`](dictionary.html#create-turtles) and [`hatch`](dictionary.html#hatch), have an implied [`without-interruption`](dictionary.html#without-interruption) around them.)

Note that the behavior of [`ask-concurrent`](dictionary.html#ask-concurrent) is completely deterministic. Given the same code and the same initial conditions, the same thing will always happen (if you are using the same version of NetLogo and begin your model run with the same random seed).
...
NetLogo features several primitives which allow a model to interact with the user. These primitives include [user-directory](dictionary.html#user-directory), [user-file](dictionary.html#user-file), [user-new-file](dictionary.html#user-new-file), [user-input](dictionary.html#user-input), [user-message](dictionary.html#user-message), [user-one-of](dictionary.html#user-one-of), and [user-yes-or-no?](dictionary.html#user-yes-or-no).
...
### [What does “Halt” mean?](#what-does-halt-mean)
...
When a link’s [`tie-mode`](dictionary.html#tie-mode) is set to “fixed” or “free” [`end1`](dictionary.html#end1) and [`end2`](dictionary.html#end2) are tied together. If the link is directed [`end1`](dictionary.html#end1) is the “root agent” and [`end2`](dictionary.html#end2) is the “leaf agent”. That is when [`end1`](dictionary.html#end1) moves (using [`fd`](dictionary.html#forward), [`jump`](dictionary.html#jump), [`setxy`](dictionary.html#setxy), etc.) [`end2`](dictionary.html#end2) also moves the same distance and direction. However when [`end2`](dictionary.html#end2) moves it does not affect [`end1`](dictionary.html#end1).
...
When the root turtle turns right or left, the leaf turtle rotates around the root turtle the same amount as if a stiff were attaching the turtles. When [`tie-mode`](dictionary.html#tie-mode) is set to “fixed” the heading of the leaf turtle changes by the same amount. If the [`tie-mode`](dictionary.html#tie-mode) is set to “free” the heading of the leaf turtle is unchanged.

The [`tie-mode`](dictionary.html#tie-mode) of a link can be set to “fixed” using the [`tie`](dictionary.html#tie) command and set to “none” (meaning the turtles are no longer tied) using [`untie`](dictionary.html#untie) to set the mode to “free” you need to: `set tie-mode "free"`.

> **Code Example:** Tie System Example

## [Multiple source files](#multiple-source-files)

The [`__includes`](dictionary.html#includes) keyword allows you to use multiple source files in a single NetLogo model.
...
When you open a model that uses the [`__includes`](dictionary.html#includes) keyword, or if you add it to the top of a model and hit the Check button, the includes menu will appear in the toolbar. From the includes menu you can select from the files included in this model.

When you open included files they appear in additional tabs. See the [Interface Guide](codetab.html#included-files-menu) for more details.

You can have anything in external source files (`.nls`) that you would normally put in the Code tab: [`globals`](dictionary.html#globals), [`breed`](dictionary.html#breed), [`turtles-own`](dictionary.html#turtles-own), [`patches-own`](dictionary.html#patches-own), [`breeds-own`](dictionary.html#turtles-own), procedure definitions, etc. Note though that these declarations all share the same namespace. That is, if you declare a global `my-global` in the Code tab you cannot declare a global (or anything else) with the name `my-global` in any file that is included in the model. `my-global` will be accessible from all the included files. The same would be true if `my-global` were declared in one of the included files.
...
The only keywords in the language are [`globals`](dictionary.html#globals), [`breed`](dictionary.html#breed), [`turtles-own`](dictionary.html#turtles-own), [`patches-own`](dictionary.html#patches-own), [`to`](dictionary.html#to), [`to-report`](dictionary.html#to-report), and [`end`](dictionary.html#end), plus [`extensions`](dictionary.html#extensions) and the experimental [`__includes`](dictionary.html#includes) keyword. (Built-in primitive names may not be shadowed or redefined, so they are effectively a kind of keyword as well.)

### [Identifiers](#identifiers)

All primitives, global and agent variable names, and procedure names share a single global case-insensitive namespace; local names ([`let`](dictionary.html#let) variables and the names of procedure inputs) may not shadow global names or each other. Identifiers may contain any Unicode letter or digit and the following ASCII characters:
...
A program consists of optional declarations ([`globals`](dictionary.html#globals), [`breed`](dictionary.html#breed), [`turtles-own`](dictionary.html#turtles-own), [`patches-own`](dictionary.html#patches-own), [`<BREED>-own`](dictionary.html#turtles-own), [`extensions`](dictionary.html#extensions)) in any order, followed by zero or more procedure definitions. Multiple breeds may be declared with separate [`breed`](dictionary.html#breed) declarations; the other declarations may appear once only.

Every procedure definition begins with [`to`](dictionary.html#to) or [`to-report`](dictionary.html#to-report), the procedure name, and an optional bracketed list of input names. Every procedure definition ends with [`end`](dictionary.html#end). In between are zero or more commands.

### [Commands and reporters](#commands-and-reporters)
...
All commands and reporters, both primitive and user-defined, take a fixed number of inputs by default. (That’s why the language can be parsed though there is no punctuation to separate or terminate commands and/or inputs.) Some primitives are variadic, that is, may optionally take a different number of inputs than the default; parentheses are used to indicate this, e.g. `(list 1 2 3)` (since the [`list`](dictionary.html#list) primitive only takes two inputs by default). Parentheses are also used to override the default operator precedence, e.g. `(1 + 2) * 3`, as in other programming languages.
...
-   [`with`](dictionary.html#with), [`with-min`](dictionary.html#with-min), [`with-max`](dictionary.html#with-max), [`at-points`](dictionary.html#at-points), [`in-radius`](dictionary.html#in-radius), [`in-cone`](dictionary.html#in-cone), [`who-are-not`](dictionary.html#who-are-not)
-   [`of`](dictionary.html#of)
-   All other primitives and user-defined procedures, including [`not`](dictionary.html#not)
-   `^`
-   `*`, `/`, `mod`
-   `+`, `-`
-   `<`, `>`, `<=`, `>=`
-   `=`, `!=`
-   [`and`](dictionary.html#and), [`or`](dictionary.html#or), [`xor`](dictionary.html#xor)
-   [`ifelse-value`](dictionary.html#ifelse-value)

When an expression contains multiple operators with the same precedence, they are read in code order left-to-right. Example: `true or false and false` is read as `(true or false) and false` and so results in `(true) and false`, which is `false`. This is different than many other programming languages where `or` has a lower precedence than `and`, so the above statement would be read as `true or (false and false)`, `true or (false)`, so `true`. You can surround expressions in parentheses to ensure you get your desired order of operations.

### [Compared to other Logos](#compared-to-other-logos)
...
-   The [`and`](dictionary.html#and) and [`or`](dictionary.html#or) reporters are special forms, not ordinary functions, and they “short circuit”, that is, they only evaluate their second input if necessary.
-   Procedures can only be defined in the Code tab, not interactively in the Command Center.
-   Reporter procedures, that is, procedures that “report” (return) a value, must be defined with [`to-report`](dictionary.html#to-report) instead of [`to`](dictionary.html#to). The command to report a value from a reporter procedure is [`report`](dictionary.html#report), not `output`.
-   When defining a procedure, the inputs to the procedure must be enclosed in square brackets, e.g. `to square [x]`.
-   Variable names are always used without any punctuation: always `foo`, never `:foo` or `"foo`. (To make this work, instead of a `make` command taking a quoted argument we supply a [`set`](dictionary.html#set) special form which does not evaluate its first input.) As a result, procedures and variables occupy a single shared namespace.
...
#### [Deeper differences](#deeper-differences)
...
-   NetLogo’s [`run`](dictionary.html#run) command works on anonymous procedures and strings, not lists (since we have no “word” data type), and does not permit the definition or redefinition of procedures.
-   Control structures such as [`if`](dictionary.html#if) and [`while`](dictionary.html#while) are special forms, not ordinary functions. You can’t define your own special forms, so you can’t define your own control structures. (You can do something similar using anonymous procedures, but you must use the `->`, `run`, and `runresult` primitives for that, you cannot make them implicit.)
